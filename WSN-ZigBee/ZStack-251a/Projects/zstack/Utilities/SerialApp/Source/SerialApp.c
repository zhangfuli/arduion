...
static void GPS(void){
  UartSend_String("The GPS ID is: ",15); 
  PERCFG = 0x02;           //PERCFG UART1选择备用地址2 UART1选择备用地址1 
      
  P1SEL = 0xc0;            //设置P1_7,P1_6用作外设功能
  U1CSR |= 0xc0;           //设置为UART方式 并允许接收数据 
  U1GCR |= 10;			       
  U1BAUD |= 59;           //波特率设为38400
...
{ 
    //URX1IF = 0;       // 清中断标志 
    //char GPSRxBuf = U1DBUF; 
   
    
    //HalUARTWrite (UART0, U1DBUF, 3); 
   //if((GPSRxBuf == '$') && (strncmp(GPSRxData, CompareString, 5) == 0)){
     // UartSend_String(GPSRxData,102); 
      //memset(GPSRxData, 0, SIZE);
    //}
    if(U1DBUF != 0x00 && U1DBUF != 0x0A){
       GPSRxData[GPScount++] = U1DBUF; 
    }
    if((U1DBUF == '$')){
       UartSend_String(GPSRxData,GPScount);
       GPScount = 0;
       memset(GPSRxData, 0, SIZE);
    }
    /*if((GPSRxBuf == '$') && (strncmp(GPSRxData, CompareString, 5) == 0)){
